...
Here, I've decided to eliminate the multiple answers situation from the question key, and only keep the "main" question for each item in the Taves scale:

```{r}
question_key_taves <- question_key_taves %>%
  filter(!grepl("_..._.", question_label_universal))
```
...
# look for duplicates - this would indicate there's some discrepancy across versions
# first, questions that share a label but have different texts
dup_label_taves3 <- question_key_taves %>% # start with the question key
  distinct(question_label_universal, question_text) %>% # include only distinct combinations of question_label and question_text
  count(question_label_universal) %>% # count how many times each question_label appears
  filter(n > 1) %>% # only include things that appear more than once (potential problems)
  data.frame() # make it a dataframe
...
# next, questions that share a text but have different labels
dup_text_taves3 <- question_key_taves %>% 
  filter(question_text != "", !is.na(question_text)) %>% # exclude questions that have no quesiton text
  distinct(question_label_universal, question_text) %>% 
  count(question_text) %>% # count how many times each question_text appears
  filter(n > 1) %>% 
...
# uncomment to look
dup_label_taves3 %>% arrange(question_label_universal)
dup_text_taves3 %>% arrange(question_text)
```
...
# uncomment to look
question_key_taves %>% # start with question_key_taves
  filter(question_label_universal %in% dup_label_taves3$question_label_universal | # include only questions that either appear in the list of duplicate labels...
           question_text %in% dup_text_taves3$question_text) %>% # ... or in the list of duplicate texts
  arrange(question_text, question_label_universal) # sort
```

5. Still, pairs of questions are associated with the same text. Let's deal with that:

```{r}
dup_text_taves3_fix <- dup_text_taves3 %>% 
  distinct() %>%
  mutate(question_text = gsub("â\u0080\u009c", "'", question_text),
         question_text = gsub("â\u0080\u009d", "'", question_text),
         question_text = gsub("^.*\\[bodily sensations e.g. goosebumps\\]$",
                              "[bodily sensations e.g. goosebumps]",
                              question_text)) %>%
  rownames_to_column("number") %>%
  mutate(question_label_universal2 = paste("taves_universal",
                                           number, sep = "_")) %>%
  select(question_label_universal2, question_text)
...
question_key_taves <- question_key_taves %>%
  mutate(question_text = gsub("â\u0080\u009c", "'", question_text),
         ...
         question_text = gsub("^.*\\[bodily sensations e.g. goosebumps\\]$",
                              "[bodily sensations e.g. goosebumps]",
                              question_text)) %>%
  full_join(dup_text_taves3_fix) %>%
  mutate(question_label_universal = 
           ifelse(question_text %in% dup_text_taves3_fix$question_text, 
                  question_label_universal2, question_label_universal)) %>%
  select(-question_label_universal2)
```
...
# look for duplicates - this would indicate there's some discrepancy across versions
# first, questions that share a label but have different texts
dup_label_taves4 <- question_key_taves %>% # start with the question key
  distinct(question_label_universal, question_text) %>% # include only distinct combinations of question_label and question_text
  count(question_label_universal) %>% # count how many times each question_label appears
  filter(n > 1) %>% # only include things that appear more than once (potential problems)
  data.frame() # make it a dataframe
...
# next, questions that share a text but have different labels
dup_text_taves4 <- question_key_taves %>% 
  filter(question_text != "", !is.na(question_text)) %>% # exclude questions that have no quesiton text
  distinct(question_label_universal, question_text) %>% 
  count(question_text) %>% # count how many times each question_text appears
  filter(n > 1) %>% 
...
# uncomment to look
dup_label_taves4 %>% arrange(question_label_universal)
dup_text_taves4 %>% arrange(question_text)
```
...
```{r}
question_key <- question_key_self %>%
  full_join(question_key_sleep) %>%
  full_join(question_key_sponth) %>%
  full_join(question_key_taves) %>%
  full_join(question_key_thinkbel)
```
...
selfv1_tidy <- packet_tidy_fun(selfv1_encoded, 
                               packet = "self", version = "1")
selfv2_tidy <- packet_tidy_fun(selfv2_encoded,
                               packet = "self", version = "2")
sleepv1_tidy <- packet_tidy_fun(sleepv1_encoded,
                                packet = "sleep", version = "1")
sleepv2_tidy <- packet_tidy_fun(sleepv2_encoded,
                                packet = "sleep", version = "2")
sponthv1_tidy <- packet_tidy_fun(sponthv1_encoded,
                                 packet = "sponth", version = "1")
sponthv2_tidy <- packet_tidy_fun(sponthv2_encoded,
                                 packet = "sponth", version = "2")
tavesv1_tidy <- packet_tidy_fun(tavesv1_encoded,
                                packet = "taves", version = "1")
tavesv2_tidy <- packet_tidy_fun(tavesv2_encoded,
                                packet = "taves", version = "2")
thinkbel1vA_tidy <- packet_tidy_fun(thinkbel1vA_encoded,
                                    packet = "thinkbel", version = "1A")
thinkbel1vB_tidy <- packet_tidy_fun(thinkbel1vB_encoded,
                                    packet = "thinkbel", version = "1B")
thinkbel2vA_tidy <- packet_tidy_fun(thinkbel2vA_encoded,
                                    packet = "thinkbel", version = "2A")
thinkbel2vB_tidy <- packet_tidy_fun(thinkbel2vB_encoded,
                                    packet = "thinkbel", version = "2B")
thinkbel3vA_tidy <- packet_tidy_fun(thinkbel3vA_encoded,
                                    packet = "thinkbel", version = "3A")
thinkbel3vB_tidy <- packet_tidy_fun(thinkbel3vB_encoded,
                                    packet = "thinkbel", version = "3B")
