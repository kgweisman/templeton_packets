---
title: 'Grappling with the "Mind metaphors" list'
...
Here we look at the "mind metaphors" non-scale (Van Elke; Packet 3) in detail. Note that this list of metaphors was never meant to be summed up like this!
...
ggplot(d_long_subscale_boot %>%
         filter(subscale == "mind metaphors"),
       aes(x = ctry, y = Mean)) +
  ...
  scale_y_continuous(limits = c(-16.5, 16.5), breaks = seq(-100, 100, 10)) +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "none") +
  labs(title = "Mean mind metaphors scores by site (Van Elk)",
       subtitle = "A higher score indicates more endorsements of mind metaphors\n(range: 0 to 27)\nError bars are bootstrapped 95% confidence intervals",
       x = "Site", color = "Site",
       y = "Mean score")
```
...
d_plot <- d_long %>%
  ...
  mutate(coding = factor(coding,
                         levels = c("-1", "1"),
                         labels = c("(REVERSED)", "")),
         question_text_short = gsub('(.{1,40})(\\s|$)',
                                    '\\1\n',
                                    paste0(question,
                                           ": ",
...
ggplot(d_plot %>%
  ...
  theme_bw() +
  labs(title = "Responses to 'Mind metaphors' (Van Elk) scale items",
       subtitle = "% corresponds to non-zero responses",
...
- Few people liked the "jar" metaphor (`meta_01`), but people disagreed with it especially strongly in VT (especially given how rarely people there used the "Strongly disagree" endpoint of the scale)! A hint of "opacity"-type stuff??
- People in CH seemed to like the "antenna" metaphor (`meta_02`).
- Lots ofo people liked the "container" metaphor (`meta_03`), especially in VT - again, a tiny hint of "opacity"? And people in CH liked this quite a bit less than others, kind of dovetailing with their endorsement of the "antenna" metaphor above.
- People in TH especially liked the "locked box" metaphor (`meta_04`) and the "furniture" metaphor (`meta_08`). "Furniture" metaphors makes me think of Maria Heim's "modularity"! But in general, another thing these both have in common is the idea that there are some people who do have access or make contributions to your mind... interesting.
- Metaphors that mention God (`meta_06`, `meta_07`) got a boost in the GH and VT, as you might expect.
...
d_long %>%
  ...
  mutate(response = factor(response),
         ...
         subscale = factor(subscale, 
                           labels = c("Mind metaphors (Van Elk)"))) %>%
  distinct() %>%
  ...
  theme_bw() +
  labs(title = "Distributions of how many times participants endorsed each response option for the 'Mind metaphors' (Van Elk) scale items",
