...
d_tidy3 <- d_tidy2 %>% 
  ...
  mutate(response_coded = 
           ifelse(is.na(byhand_coding) == TRUE, response,
                  ifelse(byhand_subscale %in% 
                           c("exwl", "her_posey_losch", "minw_epistemic",
                             "minw_inanimate", "minw_life_events", 
                             "minw_mental_states", 
                             "minw_selves_souls_world"),
                         ifelse(response == 0, 1,
                                ifelse(response == 1, 0, NA)),
