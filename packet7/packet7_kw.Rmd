...
subtitle: "Last updated: 2019-05-07"
output:
  html_notebook:
    toc: yes
    toc_depth: 4
...
_ALSO NOTE: In Thailand this packet was missing one question on the Porosity (Dulin) scale. I have rescaled Thai participants' Porosity scores to run from 0-32 in order to match the other sites._
...
```{r}
r1 <- lmer(experience ~ abs * p7_ctry * scale + (1 | p7_subj),
           # contrasts = list(p7_ctry = cbind(GH = c(-1, 1, 0, 0, 0),
           #                                  TH = c(-1, 0, 1, 0, 0),
           #                                  CH = c(-1, 0, 0, 1, 0),
           #                                  VT = c(-1, 0, 0, 0, 1))),
           d_std_h1)
...
```{r}
r2_por <- lmer(experience ~ por * p7_ctry * scale + (1 | p7_subj),
           # contrasts = list(p7_ctry = cbind(GH = c(-1, 1, 0, 0, 0),
           #                                  TH = c(-1, 0, 1, 0, 0),
           #                                  CH = c(-1, 0, 0, 1, 0),
           #                                  VT = c(-1, 0, 0, 0, 1))),
...
```{r}
r2_mm <- lmer(experience ~ mm * p7_ctry * scale + (1 | p7_subj), 
           # contrasts = list(p7_ctry = cbind(GH = c(-1, 1, 0, 0, 0),
           #                                  TH = c(-1, 0, 1, 0, 0),
           #                                  CH = c(-1, 0, 0, 1, 0),
           #                                  VT = c(-1, 0, 0, 0, 1))),
...
```{r, fig.width = 6, fig.asp = 0.4, include = F}
...
d_std_h6 %>%
  spread(scale, experience) %>%
  select(p7_subj, abs, por, dse) %>%
  data.frame() %>%
...
d_por <- d0 %>%
  ...
  gather(question, response, -c(p7_ctry, p7_subj)) %>%
  mutate(val = case_when(question %in% 
                           c("p7_por_job.wish", "p7_por_conslt.unseen", 
                             "p7_por_mircl.prayer", "p7_por_pry.dead.back", 
                             "p7_por_fall.in.lov", "p7_por_thgs.heal",
                             "p7_por_visualization") ~ 
                           "positive",
                         question %in% 
                           c("p7_por_thgs.hrt", "p7_por_wifi.thgs", 
                             "p7_por_angr.cntrl", "p7_por_sprt.envy", 
                             "p7_por_read.thgs", "p7_por_stre.spoil", 
                             "p7_por_spkn.curse", "p7_por_curse.sick", 
                             "p7_por_sprt.put.thgs") ~ 
                           "negative",
                         TRUE ~ NA_character_),
         val = factor(val, levels = c("negative", "positive")))

# contrasts(d_por$p7_ctry) <- contrasts_ctry_eff
contrasts(d_por$val) <- cbind("_neg_pos" = c(0, 1))
```

```{r, fig.width = 4, fig.asp = 0.7, include = T}
por_val_plot_a <- d_por %>% 
  filter(!is.na(val)) %>%
  group_by(p7_ctry, p7_subj, val, val) %>%
  summarise(mean_score = mean(response, na.rm = T),
            sum_score = sum(response, na.rm = T)) %>%
  ungroup() %>%
  ggplot(aes(x = val, y = mean_score, color = p7_ctry)) +
  facet_grid(~ p7_ctry) +
  geom_jitter(alpha = 0.2) +
  geom_pointrange(data = . %>% group_by(p7_ctry, val, val) %>%
  ...
  labs(title = "Mean responses to Porosity (Dulin) questions by valence",
       subtitle = "\nValence coded by TML\nResponse options: 0 = it does not happen, 1 = it might happen, 2 = it definitely happens\nError bars are bootstrapped 95% confidence intervals",
       x = "Valence", 
       y = "Mean response")

por_val_plot_b <- d_por %>% 
  filter(!is.na(val)) %>%
  group_by(p7_ctry, p7_subj, val, val) %>%
  summarise(mean_score = mean(response, na.rm = T),
            sum_score = sum(response, na.rm = T)) %>%
  ungroup() %>%
  group_by(p7_ctry, val, val) %>%
  multi_boot_standard(col = "mean_score", na.rm = T) %>%
  ungroup() %>%
  ggplot(aes(x = val, color = p7_ctry)) +
...
plot_grid(por_val_plot_a, por_val_plot_b, ncol = 1)
```
...
d_por %>% 
  filter(!is.na(val)) %>%
  group_by(p7_ctry, p7_subj, val, question) %>%
  summarise(mean_score = mean(response, na.rm = T),
            sum_score = sum(response, na.rm = T)) %>%
  ungroup() %>%
  left_join(var_key %>% select(question, question_text, order)) %>%
  mutate(question_text = str_wrap(paste0(toupper(val), ": ", 
                                         question_text), 35)) %>%
  ggplot(aes(x = p7_ctry, 
             y = mean_score, color = p7_ctry)) +
  facet_wrap(. ~ question_text, ncol = 3) +
  geom_point(alpha = 0.05,
             position = position_jitterdodge(jitter.width = 2,
                                             jitter.height = 0.3,
                                             dodge.width = 0.5)) +
  geom_pointrange(data = . %>% 
                    group_by(p7_ctry, val, question, question_text) %>%
                    multi_boot_standard(col = "mean_score", na.rm = T) %>%
                    ungroup(),
                  aes(y = mean, ymin = ci_lower, ymax = ci_upper),
...
d_por %>% 
  ...
  ggplot(aes(x = p7_ctry, 
             y = mean_score, color = p7_ctry)) +
  facet_wrap(. ~ question_text, ncol = 3, scales = "free") +
  geom_pointrange(aes(y = mean, ymin = ci_lower, ymax = ci_upper),
                  position = position_dodge(width = 0.5),
                  show.legend = T) +
  scale_color_brewer(palette = "Dark2", guide = "none") +
...
d_mm %>% 
  ...
  ggplot(aes(x = emo, 
             y = mean_score, color = p7_ctry)) +
  facet_grid(str_wrap(question_type, 20) ~ p7_ctry) +
  geom_point(alpha = 0.05,
             position = position_jitterdodge(jitter.width = 2,
                                             jitter.height = 0.3,
                                             dodge.width = 0.5)) +
...
d_mm %>% 
  ...
  group_by(p7_ctry, val, emo, 
                    ...
                    multi_boot_standard(col = "mean_score", na.rm = T) %>%
                    ungroup() %>%
  ggplot(aes(x = emo, color = p7_ctry, shape = emo)) +
  facet_grid(str_wrap(question_type, 20) ~ p7_ctry, scales = "free") +
  geom_pointrange(aes(y = mean, ymin = ci_lower, ymax = ci_upper),
                  position = position_dodge(width = 0.5),
                  show.legend = T) +
  scale_color_brewer(palette = "Dark2", guide = "none") +
...
```{r}
ctl_plot_a <- d_ctl_norev %>%
  mutate(scale = recode(scale,
                        "hthk" = "Need for Cognition",
                        "wob" = "Sense of Control")) %>%
  ggplot(aes(x = p7_ctry, y = score_norev, color = p7_ctry)) +
  facet_grid(scale ~ .) +
  geom_jitter(alpha = 0.2, height = 0.1, show.legend = F) +
  geom_pointrange(data = . %>%
                    group_by(p7_ctry, scale) %>%
                    multi_boot_standard("score_norev", na.rm = T) %>%
                    ungroup(),
                  aes(y = mean, ymin = ci_lower, ymax = ci_upper),
                  color = "black", show.legend = F, fatten = 2) +
  scale_color_brewer(palette = "Dark2") +
  labs(title = "Site differences in 'control' scales",
       subtitle = "NOTE: Omitting reverse-coded items\nError bars are bootstrapped 95% CIs",
       x = "Site", y = "Score (no reverse-coded items)")

ctl_plot_b <- d_ctl_norev %>%
  mutate(scale = recode(scale,
                        "hthk" = "Need for Cognition",
                        "wob" = "Sense of Control")) %>%
  group_by(p7_ctry, scale) %>%
...
plot_grid(ctl_plot_a, ctl_plot_b, ncol = 2)
```
...
#   scale_color_brewer(palette = "Dark2") +
#   labs(title = "Site differences in 'control' scales",
#        subtitle = "NOTE: Including reverse-coded items\nError bars are bootstrapped 95% CIs",
#        x = "Site", y = "Score (including reverse-coded items)")
# 
# ctl_plot_d <- d_ctl %>%
#   mutate(scale = recode(scale,
#                         "hthk" = "Need for Cognition",
...
```{r, fig.width = 4, fig.asp = 0.8, include = T}
# plot_grid(ctl_plot_c, ctl_plot_d, ncol = 2)
```
...
d0 %>%
  ...
  geom_label(data = . %>% group_by(p7_ctry) %>%
             ...
             aes(x = 17, y = 0.08, 
                 label = paste0("median: ", round(median, 2))),
             hjust = 0.5, fill = "white", alpha = 0.5, size = 3) +
  scale_x_continuous(breaks = seq(0, 32, 16)) +
  scale_fill_brewer(palette = "Dark2") +
  labs(title = "Absorption", 
...
```{r}
d_alpha <- data.frame(
  `Daily Spiritual Experiences` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_dse_"))),
  `Spiritual Events` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_se_")) %>% remove_missing()),
  `Uncommon Events` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_unev_"))),
  `Extrasensory` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_exsen_"))),
  `Absorption` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_abs_"))),
  `Porosity (Dulin)` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_por_"))),
  `Martha Story` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_mm_"))),
  `Ways of being` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_wob_"))),
  `Hard thinking` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_hthk_"))),
  `Ways of being (no rev)` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_wob_"), -contains("reverse"))),
  `Hard thinking (no rev)` = alpha_by_ctry_fun(
    d0 %>% select(p7_ctry, contains("_hthk"), 
                  -c("p7_hthk_not.fun", "p7_hthk_lil.challeng", 
  ...
  mutate(sample = 
         ...
         scale = gsub("\\.", " ", scale),
         scale = factor(scale,
                        levels = c("Daily Spiritual Experiences", 
                                   "Spiritual Events",
                                   "Uncommon Events", "Extrasensory",
                                   "Absorption", "Porosity (Dulin)",
                                   "Martha Story", "Ways of being",
                                   "Ways of being (no rev)",
                                   "Hard thinking", "Hard thinking (no rev)")))
```
...
d_alpha %>%
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  labs(title = "Reliability by site and overall",
       subtitle = "Cronbach's alpha = 0.8 (black dotted line) is a classic cutoff for 'very good' reliability;\nin practice, alpha < 0.7 (gray dotted line) is often considered problematic",
...
#   rename_all(funs(gsub("p7_", "", .))) %>%
#   rename_all(funs(gsub("_total", "", .))) %>%
#   cor.ci(plot = F)
...
d_cor %>%
  ...
  scale_fill_distiller(palette = "RdYlBu", limits = c(-1, 1),
                       guide = guide_colorbar("Pearson's r",
                                              barheight = 10,
                                              barwidth = 1)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        axis.title = element_blank()) +
  labs(title = "Correlations between measures included in Packet 7") #,
       # subtitle = "After standardizing each measure within site")
```
